...
# =========================
# تنسيقات CSS احترافية وسهلة الاستخدام
# =========================
st.markdown("""
<style>
...
if menu == "🏠 لوحة التحكم":
    ...
    st.markdown('<p class="help-text">💡 هنا يمكنك رؤية ملخص سريع عن حالة النظام والمبيعات والمخزون.</p>', unsafe_allow_html=True)
...
# =========================
    ...
    st.markdown('<p class="help-text">➕ أضف منتجاتك هنا (الاسم، السعر، المخزون). يمكنك أيضًا مسح جميع المنتجات للبدء من جديد.</p>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        # --- >>> زر مسح جميع المنتجات <<< ---
        if st.button("🗑️ مسح جميع المنتجات", use_container_width=True, type="secondary"):
            st.session_state.confirm_delete = True
            
        if st.session_state.get("confirm_delete", False):
            st.error("⚠️ أنت على وشك حذف جميع المنتجات والمبيعات!")
            confirm = st.checkbox("أنا متأكد، أريد حذف كل شيء")
            if confirm:
                cur.execute("DELETE FROM products")
                cur.execute("DELETE FROM sales")
                conn.commit()
                st.session_state.confirm_delete = False
                st.success("✅ تم مسح جميع المنتجات والمبيعات بنجاح! النظام الآن فارغ.")
                st.rerun()
...
# =========================
    ...
    st.markdown('<p class="help-text">🛍️ اختر المنتجات وأضفها إلى سلة المشتريات. عند الانتهاء، اضغط "إتمام عملية البيع".</p>', unsafe_allow_html=True)
...
# =========================
    ...
    st.markdown('<p class="help-text">📋 هنا يمكنك الاطلاع على جميع عمليات البيع التي تمت في النظام.</p>', unsafe_allow_html=True)
    sales = pd.read_sql("SELECT * FROM sales", conn)
